...
function diffClauses(
  ...
  if (!sameIds(beforeWorkplaceIds, afterWorkplaceIds)) {
    ...
    if (beforeLabel !== null && afterLabel !== null) {
      clauses.push(`Workplace changed from ${beforeLabel} to ${afterLabel}`);
    } else {
      clauses.push("Workplace changed");
    }
  }
  ...
  if (beforeEnabled !== afterEnabled) {
    clauses.push(`Overtime ${afterEnabled ? "enabled" : "disabled"}`);
  }
  ...
  if (beforeOvertimeHours !== afterOvertimeHours) {
    clauses.push(
      `Overtime hours changed from ${formatHours(beforeOvertimeHours)} to ${formatHours(afterOvertimeHours)}`,
    );
  }
  ...
  if (beforeLeaveHours !== afterLeaveHours) {
    clauses.push(
      `Leave hours changed from ${formatHours(beforeLeaveHours)} to ${formatHours(afterLeaveHours)}`,
    );
  }

  if ((before?.note || "") !== (after?.note || "")) {
    clauses.push("Note updated");
  }

  return clauses;
}

// Compact "what was there" fragment for create/delete, e.g. "K5, 2.5h overtime".
function snapshotFragment(
  ...
  if (snapshot.overtime_enabled && Number(snapshot.overtime_hours) > 0) {
    parts.push(`${formatHours(Number(snapshot.overtime_hours))}h overtime`);
  }
  if (Number(snapshot.leave_hours) > 0) {
    parts.push(`${formatHours(Number(snapshot.leave_hours))}h leave`);
  }
...
function summarize(
  ...
  const dateFragment = isValidWorkDate(workDate) ? ` for ${workDate}` : "";

  if (log.event_type === "create") {
    const extra = snapshotFragment(detail.after, workplaces);
    return `${userLabel} created a schedule record${dateFragment}${extra ? ` (${extra})` : ""}`;
  }

  if (log.event_type === "delete") {
    const extra = snapshotFragment(detail.deleted, workplaces);
    return `${userLabel} deleted the schedule record${dateFragment}${extra ? ` (${extra})` : ""}`;
  }

  // update
  const clauses = diffClauses(detail.before, detail.after, workplaces);
  if (clauses.length === 0) {
    return "Record updated (no recognizable field changes)";
  }
  return clauses.join(", ");
}
